...
TH1D* PartonMatch_1D(std::string dir, TFile* f, std::string fname,  std::string var, std::string cut,  double xbin, double xmin, double xmax, std::string xlabel, std::string postfix, Color_t color = kBlue, int linestyle = 1,bool save = false){
  ...
  delete t;
  if(save)cvs->SaveAs((savedir+"/"+fname+"_"+var_+"_"+cut+".pdf").c_str());
...
void Plot_efficiency(){
  ...
  c3->SaveAs((savedir+"/"+"efficiency_loose.pdf").c_str());
  ...
  c3_m->SaveAs((savedir+"/"+"efficiency_med.pdf").c_str());
  ...
  c3_t->SaveAs((savedir+"/"+"efficiency_tight.pdf").c_str());
  ...
  c_sig->SaveAs((savedir+"/"+"efficiency_sig.pdf").c_str());
  ...
  c_bkg->SaveAs((savedir+"/"+"efficiency_bkg.pdf").c_str());
  ...
  c_sb->SaveAs((savedir+"/"+"efficiency_sb.pdf").c_str());
  ...
  c->SaveAs((savedir+"/"+"ROC.pdf").c_str());
...
void Plot_efficiency_dMHigs10_dMHigs20(){
  ...
  fsig = "Rad_HHto4b_HW10";
  fbkg = "ZPrimeToTT_HW10";
  TFile *fs = new TFile(("fMatch_"+fsig+"_"+postfix+".root").c_str());
  TFile *fb = new TFile(("fMatch_"+fbkg+"_"+postfix+".root").c_str());

  TH1D* h_sig_t = PartonMatch_1D(dir,fs,fsig,var,tight_cut,xbin,xmin,xmax,xlabel,postfix,color1,1,save);
  TH1D* h_bkg_t = PartonMatch_1D(dir,fb,fbkg,var,tight_cut,xbin,xmin,xmax,xlabel,postfix,color2,1,save);

  deltaHiggsMass = "HiggsWin20";
  fsig2 = "Rad_HHto4b_HW20";
  fbkg2 = "ZPrimeToTT_HW20";
  TFile *fs2 = new TFile(("fMatch_"+fsig2+"_"+postfix+".root").c_str());
  TFile *fb2 = new TFile(("fMatch_"+fbkg2+"_"+postfix+".root").c_str());
  ...
  fsig3 = "Rad_HHto4b_HW30";
  fbkg3 = "ZPrimeToTT_HW30";
  TFile *fs3 = new TFile(("fMatch_"+fsig3+"_"+postfix+".root").c_str());
  TFile *fb3 = new TFile(("fMatch_"+fbkg3+"_"+postfix+".root").c_str());
  ...
  //double Ssig_t = 1972; //number of matched sig with deltaR<0.05, fj_nMj>=2,
  //double Sbkg_t = 1912; //number of matched bkg with deltaR<0.05, fj_nMj>=2,
  ...
  double Ssig_t = 30041; //number of matched sig with deltaR<0.05, fj_pt>300, fj_nMj>=2, fj_nbtagMj>=2
  double Sbkg_t = 17335; //number of matched bkg with deltaR<0.05, fj_pt>300, fj_nMj>=2, fj_nbtagMj>=2
  ...
  TGraph *gr1_t = new TGraph(sig_t_nbins,x_t,sig_t_eff);
  gr1_t->SetName("gr1_t");
  gr1_t->SetTitle("RadionHH - HiggsWin10");
  gr1_t->SetMarkerStyle(21);
  gr1_t->SetMarkerColor(kGreen);
  gr1_t->SetDrawOption("AP");
  gr1_t->SetLineColor(kBlue);
  ...
  TGraph *gr2_t = new TGraph(sig_t_nbins,x_t,bkg_t_eff);
  gr2_t->SetName("gr2_t");
  gr2_t->SetTitle("ttjets - HiggsWin10");
  gr2_t->SetMarkerStyle(22);
  gr2_t->SetMarkerColor(kRed+4);
  gr2_t->SetDrawOption("P");
  gr2_t->SetLineColor(kRed);
  ...
  gr1_t2->SetTitle("RadionHH - HiggsWin20");
  ...
  c3_t->SaveAs((savedir+"/"+"efficiency_tight_deltaHiggsMass.pdf").c_str());
  ...
  c_sig->SaveAs((savedir+"/"+"efficiency_sig_deltaHiggsMass.pdf").c_str());
  ...
  c_bkg->SaveAs((savedir+"/"+"efficiency_bkg_deltaHigssMass.pdf").c_str());
  ...
  gr1_t->SetTitle("HH - dR<0.05 - Hwin10");
  ...
  mg_sb->GetYaxis()->SetTitle("Efficiency");
  mg_sb->SetTitle("after loose matching");
  c_sb->BuildLegend(0.6,.65,.9,.9);

  c_sb->SaveAs((savedir+"/"+"efficiency_sb_deltaHiggsMass.pdf").c_str());
  ...
  gr3_t2->SetTitle("SD_HiggsWin20");
  gr3_t2->SetLineColor(kBlue);
  gr3_t2->SetLineWidth(2);
  gr3_t2->SetLineStyle(2);
  ...
  mg_ROC->GetXaxis()->SetTitle("Eff_{sig}");
  mg_ROC->GetYaxis()->SetTitle("1-Eff_{bkg}");
  mg_ROC->GetXaxis()->SetLimits(0.,1.);
  mg_ROC->GetYaxis()->SetLimits(0.,1.);
  mg_ROC->SetTitle("ROC Curves");
  ...
  */
  ...
  c->SaveAs((savedir+"/"+"ROC_deltaHiggsMass.pdf").c_str());
