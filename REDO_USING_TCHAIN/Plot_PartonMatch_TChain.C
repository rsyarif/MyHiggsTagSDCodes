...
TH2D* Plot_PartonMatch_2D(std::string dir, std::string fname,  std::string var1, std::string var2, std::string cut,  double xbin, double xmin, double xmax,double ybin, double ymin, double ymax, std::string xlabel,std::string ylabel, std::string DrawOption, std::string postfix,bool save = false){
  ...
  TFile *f = new TFile(("fMatch_"+fname+"_"+postfix+".root").c_str());
  ...
  cvs->SetRightMargin(.35);
  gStyle->SetOptStat("nemrous");
  h2->Draw(DrawOption.c_str());

  delete t;
  if(save)cvs->SaveAs((savedir+"/2D_"+fname+"_"+var1_+"_"+var2_+"_"+cut+"_"+DrawOption+".pdf").c_str());
  return h2;
}
...
//   canvas->SaveAs((savedir+"/loose_match.pdf").c_str());
//   canvas->Write();
...
//   canvas2->SaveAs((savedir+"/medium_match.pdf").c_str());
//   canvas2->Write();
...
//   canvas3->SaveAs((savedir+"/tight_match.pdf").c_str());
//   canvas3->Write();
...
//   canvas4->SaveAs((savedir+"/sig_match.pdf").c_str());
//   canvas4->Write();
...
//   canvas5->SaveAs((savedir+"/bkg_match.pdf").c_str());
//   canvas5->Write();
...
//   leg->Draw("SAME");
//   gPad->Update();
//   if(save)canvas_l->SaveAs((savedir+"/loose_match"+"_"+var+".pdf").c_str());
...
//   leg->Draw("SAME");
//   gPad->Update();
//   if(save)canvas_m->SaveAs((savedir+"/medium_match"+"_"+var+".pdf").c_str());
...
//   leg->Draw("SAME");
//   gPad->Update();
//   if(save)canvas_t->SaveAs((savedir+"/tight_match"+"_"+var+".pdf").c_str());
...
void makeHistos_Fj_gen_pt_2plots(bool save = false, std::string dir="", std::string fsig="", std::string cut="", double xbin=50, double xmin=0, double xmax=100, std::string postfix=""){
  ...
  TFile *f_l = new TFile(("fMatch_"+fsig+"_"+postfix+".root").c_str());
 ...
 //--------loose-------
  ...
  gPad->Update();
  if(save)canvas_l->SaveAs((savedir+"/fj_gen_2plots_loose_match"+"_"+fsig+"_"+var+"_"+cut+".pdf").c_str());
  ...
  gPad->Update();
  if(save)canvas_m->SaveAs((savedir+"/fj_gen_2plots_medium_match"+"_"+fsig+"_"+var+"_"+cut+".pdf").c_str());
  ...
  gPad->Update();
  if(save)canvas_t->SaveAs((savedir+"/fj_gen_2plots_tight_match"+"_"+fsig+"_"+var+"_"+cut+".pdf").c_str());
...
//   leg->Draw("SAME");
//   gPad->Update();
//   if(save)canvas_l->SaveAs((savedir+"/loose_match"+"_logChi"+".pdf").c_str());
...
//   leg->Draw("SAME");
//   gPad->Update();
//   if(save)canvas_m->SaveAs((savedir+"/medium_match"+"_logChi"+".pdf").c_str());
...
//   leg->Draw("SAME");
//   gPad->Update();
//   if(save)canvas_t->SaveAs((savedir+"/tight_match"+"_logChi"+".pdf").c_str());
...
void  makeHistos_sig_bkg_2plots(std::string dir, std::string fsig, std::string fbkg, std::string var, std::string cut, double xbin, double xmin, double xmax, std::string xlabel, bool save = false, bool isLeg = true){
  ...
  if(save)canvas_t->SaveAs((savedir+"/sig_bkg_2plots_tight_match"+"_"+var+"_"+cut+".pdf").c_str());
...
void Alakazam(){
  ...
  fsig = "Rad_HHto4b_HW20";
  fbkg = "ZPrimeToTT_HW20";
