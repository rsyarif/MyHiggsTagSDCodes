{
  ...
  int nbins = 40; float xmin = 0; float xmax = 800;
  ...
  c1->SaveAs("rootfiles/QCD470-600_Mj_pt_abs(Fj_flavour)!=5&&abs(Fj_flavour)!=4_.eps");

  t->Draw("Mj_pt>>h2","Mj_isBtag==1&&abs(Fj_flavour)!=5&&abs(Fj_flavour)!=4");

  c1->SaveAs("rootfiles/QCD470-600_Mj_pt_Mj_isBtag==1&&abs(Fj_flavour)!=5&&abs(Fj_flavour)!=4_.eps");


  //t->Draw("Mj_pt>>h1","abs(Mj_gen_matched_pdgID)<99999&&abs(Mj_gen_matched_pdgID)!=5&&abs(Mj_gen_matched_pdgID)!=4");

  //c1->SaveAs("rootfiles/QCD470-600_Mj_pt_abs(Mj_gen_matched_pdgID)<99999&&abs(Mj_gen_matched_pdgID)!=5&&abs(Mj_gen_matched_pdgID)!=4_.eps");

  //t->Draw("Mj_pt>>h2","abs(Mj_gen_matched_pdgID)<99999&&abs(Mj_gen_matched_pdgID)!=5&&abs(Mj_gen_matched_pdgID)!=4&&Mj_isBtag==1");

  //c1->SaveAs("rootfiles/QCD470-600_Mj_pt_abs(Mj_gen_matched_pdgID)<99999&&abs(Mj_gen_matched_pdgID)!=5&&abs(Mj_gen_matched_pdgID)!=4&&Mj_isBtag==1_.eps");

  //t->Draw("Mj_pt>>h1","Mj_dR_gen<0.05&&abs(Mj_gen_matched_pdgID)<99999&&abs(Mj_gen_matched_pdgID)!=5&&abs(Mj_gen_matched_pdgID)!=4");

  //c1->SaveAs("rootfiles/QCD470-600_Mj_pt_Mj_dR_gen<0.05&&abs(Mj_gen_matched_pdgID)<99999&&abs(Mj_gen_matched_pdgID)!=5&&abs(Mj_gen_matched_pdgID)!=4_.eps");
  ...
  //c1->SaveAs("rootfiles/QCD470-600_Mj_pt_Mj_dR_gen<0.05&&abs(Mj_gen_matched_pdgID)<99999&&abs(Mj_gen_matched_pdgID)!=5&&abs(Mj_gen_matched_pdgID)!=4&&Mj_isBtag==1_.eps");
  ...
  for (int i =0 ; i<nbins;i++){
    ...
    v2_std[i] = sqrt(v2[i]);
    ratio[i] = (v2[i]==0? 0 : v2[i]/v1[i]);
    ratio_err_x[i] = (xmax - xmin)/nbins *0.5;
    ratio_err_y[i] = ratio[i] * sqrt( (v1_std[i] / v1[i])**2  + (v2_std[i] / v2[i])**2 ) ;
    x[i] = xmin + i * (xmax-xmin)/nbins ;
  }

  TGraphErrors *gr = new TGraphErrors(nbins,x,ratio,ratio_err_x,ratio_err_y);
  gr->Draw("AP");
  gr->SetTitle("ratio of #frac{IVF matched non-b/c Mj}{non-b/c Mj}");
  gr->GetXaxis()->SetTitle("p_{T}");

  c1->SetGridy();
  c1->SaveAs("rootfiles/QCD_Pt-470to600_IVFfakerate_PtDependance.eps");
}
