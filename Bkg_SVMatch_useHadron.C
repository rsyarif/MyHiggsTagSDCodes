	...
	for(int iGenMj=0; iGenMj < nGenPruned; iGenMj++){
	  ...
	  if(dR_Mj<MjCone){
	    ...
	    if(fabs(Mj_dR_gen[iMj])>dR_Mj){ //TO MAKE SURE THIS RECORDS THE SMALLEST deltaR!!
	      ...
	      if(abs(GenPruned_pdgID[iGenMj])<4 || abs(GenPruned_pdgID[iGenMj]) >18){ //only match to udsg, and hadrons, skip c,b quarks and leptons
		Mj_dR_gen[iMj] = dR_Mj;
		Mj_dR_genIdx[iMj] = ib;
...
void SVMatch(bool display = false){
  ...
  string prefix= "FakeMjIVFbtagCalculation_noMjConeCut4genMatch_useHadron";
  //string prefix= "FakeMjIVFbtagCalculation_08MjConeCut4genMatch";
  ...
  fbkg = "QCD_Pt-470to600_Tune4C_13TeV_pythia8_R08_r015_KtMj_HiggsWin30_tagr07_fake02_BtagALLMj_MjIVF015_MORE_mc_subjets";
  int bkg = SVMatch(dir,fbkg,dRmax,MjCone,display,prefix);
